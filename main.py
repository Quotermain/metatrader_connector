...
from utils.get_positions import get_positionsfsdf
...
while True:
    for ticker in ALL_TICKERS:
        try:
            ...
            open_close_5min_dif = (df.high - df.low) / df.low
            cond_5min = all(open_close_5min_dif < dict_open_close_5min_dif[ticker])

            if cond_day and cond_5min:
                signal_is_sent = check_signal_is_sent(ticker)
                if not signal_is_sent:
                    acceptable_PERC_loss = dict_open_close_5min_dif[ticker] * 0.5
